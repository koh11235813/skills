...
- Goal continuation — current-session observation, 2026-09-06 (OpenCode TUI, `Muse Spark 1.3 Free`, sent with `tmux send-keys` text and `Enter` as separate calls): OpenCode has no `/goal` command. A `/goal <objective>` line is treated as an ordinary prompt: the session executed the objective once (it ran `send.sh` and replied) and stopped, with no goal indicator or continuation. As a manager-loop implementer it therefore needs the explicit "do not return until done or stalled" phase prompt plus a follow-up message per incomplete reply; see `../manager-loop.md`.
